...
class TFML:
    ...
    def dataProcess(self):
        ...
        reframed.drop(reframed.columns[[i for i in range(len(scaled[0]) + 1, 2 * len(scaled[0]))]], axis=1, inplace=True)
        print(reframed.head())
    ...
    def __init__(self,name):
        ...
        self.model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.001), loss='mean_squared_error', metrics=['accuracy'])


    def run(self, epochs=10, batch_size=72):
            dataset_size = self.train_X.shape[0]
            ...
            for epoch in range(epochs):
                ...
                for i in range(0, dataset_size, batch_size):
                    ...
                    with tf.GradientTape() as tape:
                        ...
                        loss = tf.reduce_mean(tf.keras.losses.mean_squared_error(batch_y, predictions))
                    gradients = tape.gradient(loss, self.model.trainable_variables)
                    optimizer.apply_gradients(zip(gradients, self.model.trainable_variables))
                train_loss = tf.reduce_mean(tf.keras.losses.mean_squared_error(self.train_y, self.model(self.train_X, training=False)))
                train_accuracy = tf.reduce_mean(tf.cast(tf.abs(self.train_y - self.model(self.train_X, training=True)[:, 0]) <= 0.1, tf.float32))
